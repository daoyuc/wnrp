...
- **待办（性能之外的结构性项，未动工）**：CLI 与 UI 的建站编排重复（建议抽 `core/site_service.py`）、`_valid_domain/_safe_conf_base/_nginx_path` 各两份、备份/回滚样板散落（建议 `core/file_backup.py`）、hosts「一键还原」API 已有但界面无入口（与 README 描述不符）、vhost/hosts/ini 写路径缺自动化测试。
