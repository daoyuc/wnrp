...
### 运行日志页
- **全局运行日志**：统一记录应用启动 / 退出、服务启停结果、崩溃告警、自愈守护、界面回调异常与未捕获异常等，供事后排障（状态栏只显示最后一条，历史在这里查）
- 典型条目：`phpvm 1.2.0 启动（darwin · 语言 zh_CN）`、`PHP php82 启动成功：已启动 (pid 12345)`、`Redis redis@6.2 启动失败：...`、`检测到 php-cgi 崩溃（2 次）`、`界面回调异常：TclError：...`
- 列：时间 / 级别（信息 · 成功 / 警告 / 错误，分色显示）/ 来源（`app` · `env` · `services` · `crash` · `recover` · `update` · `ui`）/ 内容；选中行在下方详情区看完整多行消息
